...
export class ReplayDriver implements IDriver {
  ...
  public writeCharacteristic(
    ...
    return new Promise<void>((resolve, reject) => {
      this.addEvent(
        RecordingEventType.writeCharacteristic,
        true,
        resolve,
        reject,
        serviceUIID,
  ...
  public readCharacteristic(
    ...
    return new Promise<ArrayBuffer>((resolve, reject) => {
      this.addEvent(
        RecordingEventType.readCharacteristic,
        true,
        resolve,
        reject,
        serviceUIID,
  ...
  public enableNotification(
    ...
    this.addEvent(
      RecordingEventType.notificationReceived,
      false,
      receive,
      null,
      serviceUIID,
    ...
    return new Promise<void>((resolve, reject) => {
      this.addEvent(
        RecordingEventType.enableNotification,
        true,
        resolve,
        reject,
        serviceUIID,
  ...
  public disableNotification(
    ...
    return new Promise<void>((resolve, reject) => {
      this.addEvent(
        RecordingEventType.disableNotification,
        true,
        resolve,
        reject,
        serviceUIID,
  ...
  protected isCallBack(eventType: RecordingEventType): boolean {
    return (
      eventType === RecordingEventType.scanFoundFn ||
  ...
  protected isSameEvent(event1: IRecordingItem, event2: IRecordingItem) {
    let result = event1.eventType === event2.eventType
    if (
      result &&
      ...
      let data1 = event1.data as IRecordCharacteristic
      let data2 = event2.data as IRecordCharacteristic
      if (
        result &&
  ...
  protected runEvent(event: IRecordingItem, queuedEvent: CallBackEvent) {
    ...
    if (event.error) {
      ...
      if (event.data) {
        data = event.data
        let eventType = RecordingEventType[event.eventType]
        if (
          eventType === RecordingEventType.readCharacteristic ||
          eventType === RecordingEventType.notificationReceived
        ) {
      ...
      if (queuedEvent.resolve) {
        try {
          ...
          this._performanceMonitor.handleError(
            'Error: while replaying event' + e
          )
        }
      }
    }
    ...
    while (
      ...
      if (this.isCallBack(RecordingEventType[event.eventType])) {
        ...
        if (this._eventCallBackMethods.length > 0) {
          for (let i = 0; i < this._eventCallBackMethods.length; i++) {
            let eventQueued = this._eventCallBackMethods[i]
  ...
  protected checkAllEventsProcessd(): boolean {
    let allDone =
      this.events.length === 0 && this._eventCallBackMethods.length === 0
  ...
  protected timeNextCheck(timeStamp?: number) {
    ...
    this._checkQueueTimerId = window.setTimeout(() => {
      this.checkQueue()
    }, duration)
  }
  ...
  protected addEvent(
    ...
    if (serviceUIID || characteristicUUID) {
      let data: IRecordCharacteristic = {
