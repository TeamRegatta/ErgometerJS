...
namespace ergometer {
  ...
  export class PerformanceMonitor {
     ...
     */
    public constructor(opts: { driver?: ble.IDriver } = {}) {
    ...
    public showInfo(info: string) {
      if (this.logLevel >= LogLevel.info) this.logEvent.pub(info, LogLevel.info)
    }
     ...
     *
     */
    protected initialize(driver: ble.IDriver) {
      if (driver) this._driver = driver
        ...
        this._driver = new ble.DriverWebBlueTooth(this)
      } else {
        this.handleError(
    ...
    protected handleRowingGeneralStatus(data: DataView) {
      const parsed: RowingGeneralStatus = {
        elapsedTime:
          utils.getUint24(
            data,
    ...
    protected handleRowingAdditionalStatus1(data: DataView) {
      const parsed: RowingAdditionalStatus1 = {
        elapsedTime:
          utils.getUint24(
            data,
      ...
      if (
        data.byteLength ===
        ble.PM_Mux_Extra_Status1_BLE_Payload.BLE_PAYLOAD_SIZE
      ) {
    ...
    protected handleRowingAdditionalStatus2(data: DataView) {
      let parsed: RowingAdditionalStatus2
      if (
    ...
    protected handleRowingAdditionalStrokeData(data: DataView) {
      const parsed: RowingAdditionalStrokeData = {
        elapsedTime:
          utils.getUint24(
            data,
    ...
    protected handleRowingSplitIntervalData(data: DataView) {
      const parsed: RowingSplitIntervalData = {
        elapsedTime:
          utils.getUint24(
            data,
    ...
    protected handleRowingAdditionalSplitIntervalData(data: DataView) {
      const parsed: RowingAdditionalSplitIntervalData = {
        elapsedTime:
          utils.getUint24(
            data,
    ...
    protected handleWorkoutSummaryData(data: DataView) {
      const parsed: WorkoutSummaryData = {
    ...
    protected handleAdditionalWorkoutSummaryData(data: DataView) {
      let parsed: AdditionalWorkoutSummaryData
      if (
        data.byteLength ===
    ...
    protected handleAdditionalWorkoutSummaryData2(data: DataView) {
      const parsed: AdditionalWorkoutSummaryData2 = {
        logEntryDate: data.getUint16(
    ...
    protected handleHeartRateBeltInformation(data: DataView) {
      const parsed: HeartRateBeltInformation = {
    ...
    protected sendCsafeCommands(byteArray: number[]): Promise<void> {
      return new Promise<void>((resolve, reject) => {
        ...
        if (byteArray && byteArray.length > 0) {
          ...
          const bytesToSend: number[] = [csafe.defs.FRAME_START_BYTE]
            .concat(byteArray)
